...
std::map<std::string, std::vector<GeomObject*>> AseParser(std::wstring _filePath)
{
	std::string line;
	std::ifstream file(_filePath);

	std::map<std::string, std::vector<GeomObject*>> result;
	...
	if (file.is_open())
	{
		while (getline(file, line))
		{
			...
			{
				result[parentObj].push_back(nowGeom);
			}
			else if (s[0] == Token[_ASEToken::TOKENR_TM_ROW0])
			{
				assert(nowGeom && "no geomatry object\n");
				nowGeom->nodeTM.r[0] = {std::stof(s[1]), std::stof(s[2]), std::stof(s[3]), 0.0f };
			}
			else if (s[0] == Token[_ASEToken::TOKENR_TM_ROW1])
			{
				assert(nowGeom && "no geomatry object\n");
				nowGeom->nodeTM.r[1] = { std::stof(s[1]), std::stof(s[2]), std::stof(s[3]), 0.0f };
			}
			else if (s[0] == Token[_ASEToken::TOKENR_TM_ROW2])
			{
				assert(nowGeom && "no geomatry object\n");
				nowGeom->nodeTM.r[2] = { std::stof(s[1]), std::stof(s[2]), std::stof(s[3]), 0.0f };
			}
			else if (s[0] == Token[_ASEToken::TOKENR_TM_ROW3])
			{
				assert(nowGeom && "no geomatry object\n");
				nowGeom->nodeTM.r[3] = { std::stof(s[1]), std::stof(s[2]), std::stof(s[3]), 1.0f };
			}
			/// 정점과 면의 갯수
			...
			{
				vertexData[std::stoi(s[1])] = DirectX::XMFLOAT3{ std::stof(s[2]) / 10, std::stof(s[4]) / 10, std::stof(s[3]) / 10 };
			}
